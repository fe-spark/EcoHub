...
| 更新列表 | 一次采集批次中判定为「有更新」的影片列表：**必须是主站已有影片（全局 mid）**；主站框架或**任一附属播放源**的剧集/线路结构变更均计入；未匹配主站的附属片不进列表 |
...
更新列表收录 **主站已有影片** 上、**任一播放源** 的实质更新（附属站用于扩展主站播放源）：
...
- **主站两层过滤**：
  ...
  2. **是否进列表**（`filterPlayStructureNotifyMIDs`）：**新片** 或 **新结构相对旧结构新增集数/新增线路**（线路名 + 集数标签，忽略链接与顺序）。
- **附属站**（扩展主站播放源）：
  1. 必须通过 match_key **匹配到主站全局 mid**，否则不进列表。
  2. playlist **新增集数/新增线路** 或 **真·首次挂上该附属源** → 进列表；**仅链接刷新/顺序变化** 不进。
  3. 每个 match_key 只通知一个最优 mid；写库前已有该站 playlist 时的 key 扩展不当首次通知。
  4. **同一影片多条目共享匹配键**（如「XXX英语」「XXX国语」同豆瓣 ID → 同 douban 匹配键）：
     同一 `(movie_key, group_index)` 槽位按落库「后写覆盖」去重，多条目并存**不算结构变化**；
     仅当某 key 本次**无内容**（源站改名/条目消失的残留）时也不进列表（陈旧 key 不通知）。
  5. **防连载剧发布期源站抖动**：集数**回退**（源站/CDN 在不同请求返回不同集数，如 16↔18）
     **不通知且不覆盖已存内容**，DB 保持已见最大集数，避免在两个版本间震荡导致同一 mid 每批反复上报。
- **批间不去重**：每批独立判定；无真结构变化时再采不应反复刷同一 mid（含多条目共享 key、源站抖动场景）。
