...
- **软件源配置与接口规范化**：
  - 软件源专有端点 `/api/provide/app` 优先探测并支持精准识别私有化状态，保留 `/api/health` 兜底兼容旧版。
  - 全局代码与文档明晰标注即将废弃的过渡端点（`/api/provide/config`、`/api/health`、`/api/index/dailyUpdates`），推荐第三方统一使用 `/api/provide/tvbox`，原生客户端统一使用 `/api/provide/app`。
