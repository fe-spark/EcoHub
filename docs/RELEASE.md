正式版 **v2.6.6**，Docker 镜像 `ghcr.io/fe-spark/ecohub:v2.6.6` 与 `ghcr.io/fe-spark/ecohub:latest`。
...
镜像内已直接内置编译好的 `migrate_match_keys` 迁移命令，无需下载任何外部脚本或配置数据库参数。

升级完成后，直接在宿主机终端执行：

```bash
docker exec -it Eco-hub migrate_match_keys
```

*(或者使用绝对路径 `docker exec -it Eco-hub /app/migrate_match_keys`)*

命令会在容器内自动读取已配置的数据库环境变量并执行流式安全迁移，输出实时进度，完成后显示 `Migration completed successfully!`。

---

### v2.6.6 核心变更

#### 1. 迁移工具直接内置入 Docker 镜像
- 镜像构建阶段自动编译 `migrate_match_keys` 并安装至 `/usr/local/bin/migrate_match_keys` 与 `/app/migrate_match_keys`。
- 生产环境容器内即可一键 `docker exec -it Eco-hub migrate_match_keys` 执行存量数据刷新，彻底告别外部脚本下载与环境依赖。
