...
## Multi-Node Deployment & Load Balancing

When deploying across multiple VPS nodes for load balancing and high concurrency, all nodes share the same MySQL and Redis instances:
- **Stateless Peer Nodes**: The system adopts a pure cloud-native stateless architecture. All nodes run as peers without complex cluster role environment variables.
- **Reverse Proxy (Nginx) Best Practices**:
  - **Public Web Browsing (`/`) and API (`/api/`)**: Configure Nginx `upstream` for multi-node load balancing (distribute concurrency by weight, paired with `proxy_next_upstream` to achieve **multiplied capacity and zero-downtime failover**).
  - **Administration UI and Uploads (`/manage`, `/api/manage/*`, `/api/upload/`)**: If cross-server shared storage (like NFS) is not configured, route admin and upload requests to the primary node with the persistent `uploads` volume.
  - **Nginx Configuration Example**:
    ```nginx
    # 1. Public API cluster load balancing
    upstream eco_cluster_api {
        server 192.168.1.10:8080 weight=1 max_fails=2 fail_timeout=5s; # Node 1 API
        server 192.168.1.11:8080 weight=2 max_fails=2 fail_timeout=5s; # Node 2 API
    }

    # 2. Public Web cluster load balancing
    upstream eco_web_cluster {
        server 192.168.1.10:3000 weight=1 max_fails=2 fail_timeout=5s; # Node 1 Web
        server 192.168.1.11:3000 weight=2 max_fails=2 fail_timeout=5s; # Node 2 Web
    }

    upstream eco_master_api {
        server 192.168.1.10:8080;                                      # Primary node API with persistent uploads
    }
    ...
    server {
        ...
        # Custom uploaded static assets (logos, banners stored on primary disk)
        location /api/upload/ {
            proxy_pass http://eco_master_api;
        }

        # High-concurrency read-only APIs load balanced across nodes
        location /api/ {
        ...
        # Default web browsing requests routed to web cluster
        location / {
