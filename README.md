...
EcoHub 是一套自托管的影视聚合系统，把多个采集源整理成可检索、可播放的私人片库。主站提供影片信息与分类，附属站为同一部影片补充播放线路，采集可按计划执行。部署后即可使用 Web 前台浏览、筛选、搜索与播放，并在管理后台维护采集源、分类与内容。对外提供 TVBox / 影视仓订阅地址 `/api/provide/config` 与 MacCMS 兼容接口 `/api/provide/vod`。发布版为 All-in-One 镜像 `ghcr.io/fe-spark/ecohub`，当前版本 **v2.1.1**。
