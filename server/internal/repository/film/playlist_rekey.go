...
func canonicalPlaylistMovieKey(film model.FilmIndex) string {
	keys := BuildMovieMatchKeysWithCategory(film.DbId, film.Name, filmIndexRootPid(film))
	if len(keys) == 0 {
		return ""
	}
	return keys[0]
...
func loadFilmIndexesByMids(mids []int64) (map[int64]model.FilmIndex, error) {
	mids = normalizeSnapshotMIDs(mids)
	out := make(map[int64]model.FilmIndex, len(mids))
	for _, chunk := range chunkSnapshotMIDs(mids, rekeyMidChunkSize) {
...
func SlavePlaylistRekeyTargets(keys []string) (map[string]string, error) {
	...
	destMidsByKey := loadMidCandidatesByMatchKeys(destKeys)

	targets := make(map[string]string, len(keys))
	for _, key := range keys {
		mids := normalizeSnapshotMIDs(midsByKey[key])
		if len(mids) != 1 {
			continue
		}
		...
		owners := normalizeSnapshotMIDs(destMidsByKey[dest])
		if len(owners) > 1 {
			continue
		}
...
func matchKeysEqual(left, right []string) bool {
	left = UniqueKeys(left)
	right = UniqueKeys(right)
...
func rewriteMatchKeysForFilms(films []model.FilmIndex, dryRun bool) (int64, error) {
	...
	existing := loadMovieMatchKeysByMids(normalizeSnapshotMIDs(mids))
	...
	if err := saveMovieMatchKeysByMid(mappings); err != nil {
		return 0, err
	}
	return int64(len(mappings)), nil
}
