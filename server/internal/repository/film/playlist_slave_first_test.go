...
func TestSlaveFirstCollect_ViewableAfterMasterCollected(t *testing.T) {
	...
	dbKey := BuildMovieMatchKeysWithCategory(12345, "仙逆", 20)[0]
	...
	masterKeys := BuildMovieMatchKeysWithCategory(0, "仙逆", 20)
...
func TestSlaveFirstCollect_KeepsCategoryIsolation(t *testing.T) {
	...
	catKey := BuildMovieMatchKeysWithCategory(0, "同名剧", 20)[0]
	plainNameKey := BuildMovieMatchKeys(0, "同名剧")[0]
	written := loadPlaylistKeys(t, gdb, "subo")
