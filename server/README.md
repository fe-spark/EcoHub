...
- **跨站匹配**（`movie_match_key`）：豆瓣 ID+片名 / 片名#大类 / 纯片名回退，用于附属站播放源对齐。豆瓣相同但片名不同不算同一部；片名去掉更新至/完结/语种/清晰度片尾，保留剧场版、3D、动态漫画。分类是 `film_index` 字段，不编进 mid；匹配键带大类以隔离同名跨类。
...
- `/api/config/basic` (站点基础信息 / 探活)
- `/api/provide/app` (EcoHub 客户端软件源)
- `/api/provide/tvbox` (TVBox / 影视仓配置)
